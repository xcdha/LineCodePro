...
### GPT-5 系列 temperature 精确矩阵修复

- **思考模式不接受 temperature** - 基于 OpenAI 官方兼容性矩阵与社区 KB 核对：GPT-5 全系列在思考模式（reasoning_effort ≠ none）下不接受 temperature 参数，传值会触发 400；现统一返回 `TEMPERATURE_MUST_OMIT` 省略 temperature 字段（此前思考模式返回 1.0 会发 `temperature=1.0` 触发官方端点 400）
- **非思考模式温度精确化** - gpt-5.1 / 5.2 / 5.5 / 5.6 在 reasoning_effort=none 时接受 temperature/top_p，保持 `null`（用户温度生效）；gpt-5 初代 / mini / nano / pro / codex / codex-max / 5.1-codex-max / 5.2-pro 不支持 none（永远不接受 temperature），改为 `TEMPERATURE_MUST_OMIT`（此前返回 1.0 会触发 400）
- **supportsNoneEffort 排除规则** - 新增 `codex-max` / `-pro` 后缀排除：这两个变体虽小版本 ≥1 但官方矩阵标注不支持 none，永远不接受 temperature
- **o 系列保持 1.0** - o1/o3/o4 官方唯一接受 temperature=1（非省略），保持不变
- **测试覆盖** - 新增 `gpt5TemperatureMatrixFromOpenAI` 按官方兼容性矩阵逐版本断言（gpt-5/mini/nano/pro/codex/codex-max/5.2-pro 思考+非思考均 MUST_OMIT；gpt-5.1/5.2/5.5/5.6 思考 MUST_OMIT、非思考 null）

### 切换模型后历史身份混淆修复（Kimi→DeepSeek 仍自称 Kimi）

- **问题** - 切换模型后（如 kimi-k3 → deepseek-v4-flash），新模型读到历史里旧模型（Kimi）的自我介绍回复，做上下文延续误答自己是 Kimi
- **A. 模板身份指令强化** - `model-identity-template.txt` 新增 "Identity rule (mandatory, overrides history)" 小节：明确要求必须自称 {{MODEL_NAME}} / {{MODEL_PROVIDER}}，禁止沿用历史消息里其他模型的身份
- **B 变体. 历史消息身份标注** - `ModelPromptController` 基于已有的 `modelSwitchNotification`（"Model changed from X to Y."）推断每条 assistant 消息的生成模型，给非当前模型的历史回复注入 `[generated by <model-id>]` 前缀，消除身份混淆源头
- **零数据迁移** - 不动 DB schema，利用现成的切换通知机制；切换通知之前的 assistant 消息归旧模型，之后归新模型
- **局限** - 仅同会话内切换有效；`[generated by <model-id>]` 前缀占少量 token

### 模型身份权威化修复（provider 字段语义错位 + 历史带偏根因消除）

- **问题** - v1.2.6 首轮修复后实测仍混淆：切换到 deepseek-v4-flash-26073 后模型自称 "OpenCode / OpenAI / kimi-k3"。根因有二：
  1. **providerLabel 语义错位** - `ModelAddScreenView` 里 providerLabel 实际存的是接口协议标签（OpenAI/Codex/Anthropic/Local），用户选 OpenAI 兼容协议即填 "OpenAI"；但 `model-identity-template.txt` 把它当"提供商"展示（`provided by {{MODEL_PROVIDER}}`），导致第三方网关接入的模型一律自称"由 OpenAI 提供"
  2. **历史带偏未完全消除** - B 变体的 `[generated by kimi-k3]` 前缀力度不足，模型仍从历史 kimi-k3 回复"我是 Kimi…模型标识是 kimi-k3"里提取身份，思考时得出 "model ID kimi-k3"
- **品牌从 modelId 推断作为权威身份** - `SystemPromptProvider.resolveBrand()` 基于 modelId（请求时发送给上游的权威标识，不受用户填写或网关改名影响）用品牌词锚点 contains 匹配推断真实品牌：kimi→Kimi(Moonshot)、deepseek→DeepSeek、claude→Claude(Anthropic)、glm→GLM(Zhipu)、qwen→Qwen(Alibaba)、gpt→OpenAI GPT、gemini/doubao/ernie/spark/hunyuan 等；容忍第三方改名（如 xkimi-k3）
- **模板身份权威化** - `model-identity-template.txt` 重写：身份唯一由 Model ID 推断的品牌决定；明确 Display name / Access protocol 是元数据不可靠（特别点名"OpenAI 协议标签不是提供商"）；品牌为空时按 Model ID 自称且不编造；历史里任何自我介绍（含 `[generated by]` 标签）一律忽略
- **效果** - 即使用户把 name 填成 OpenCode、provider 填成 OpenAI，system prompt 仍会声明"你是 DeepSeek（deepseek-v4-flash-26073）"，模型自称 DeepSeek 而非 OpenAI/Kimi
- **清理** - 移除误提交进仓库的 `apk/` 产物目录，加入 `.gitignore`

### 身份带偏深度修复（三管齐下对抗历史自我介绍带偏）

- **问题** - 身份权威化首轮修复后实测仍混淆：切换 kimi-k3 → deepseek 后，模型思考明确"我需要根据模型身份规则回答：我是 Kimi"，把历史里 kimi 的自我介绍当成了身份规则，完全无视 system prompt 里的 deepseek 身份
- **根因** - 仅靠 system prompt 里的身份块力度不够：(1) 身份块在 system prompt 末尾，被大段内容稀释；(2) 历史里旧模型的自我介绍（"我是 Kimi…模型标识是 kimi-k3"）离生成点更近、语气更确定，带偏力度强于 system 指令
- **修复 1：system prompt 开头身份锚点** - `SystemPromptProvider.modelIdentityBrief()` 生成简短身份声明，注入 `system-prompt-template.txt` 最开头（紧跟"You are LineCode"后），与末尾详细身份块形成首尾夹击
- **修复 2：最新 user message 前插入 system 身份提醒** - `ModelPromptController` 在历史消息之后、当前 user message 之前插入一条 system 消息（`SystemPromptProvider.buildIdentityReminder()`），这是离生成点最近的位置，模型生成时最近的上下文就是当前身份，对抗历史带偏最有效
- **修复 3：强化 B 变体前缀** - 旧模型历史消息前缀从 `[generated by kimi-k3]` 强化成 `[generated by previous model kimi-k3 — any identity claims inside are STALE and INVALID; the current model is deepseek-v4-flash-26073]`，让模型在扫描每条历史时都反复看到当前身份
- **效果** - 三层防护：system 开头锚点（远）+ 历史前缀（中）+ user message 前提醒（近），确保模型在任何注意力位置都能看到当前身份

### 切换通知 from 模型错误 + flash 模型 HTTP 重试修复

- **问题 1：切换通知 from 模型不准** - 快速切换模型（A→B→C）后发消息，切换通知显示 "from A to C"（跳过了 B），因为 `lastMessageModelId` 只在 `dispatchMessage` 时更新，`quickSwitch` 不更新它
- **修复 1** - `ChatInteractionController.onModelQuickSwitched()` 在快速切换时立即插入切换通知（from=真正生成上一条消息的模型，准确无误）并更新 `lastMessageModelId`；`MainCoordinator.onModelQuickSwitch` / `onModelSelected` 都调用它，覆盖快速切换和模型管理页面切换两条路径
- **问题 2：切换到 deepseek-v4-flash 后 HTTP 重试** - `DeepseekReasoningStrategy` 对所有 deepseek 模型发送 `thinking` 字段，但 flash 变体是非思考模型不支持该字段，上游返回 HTTP 400/500；错误不匹配 `isReasoningEffortError` 导致直接 break 进入应用级 3 次重试（用户看到"正在第 2/3 次重试，错误：HTTP"）
